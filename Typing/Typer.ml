...
(* fonction pour l'appel des méthodes *)
 let call_function methode args gen_env classe =
 let am= (Hashtbl.find gen_env.classes classe).methodes in
...
=======
	 if type_apparent <> pere then verif_ref_type gen_env type_apparent pere;
	 (* else pere.tid = "Object"  raise((InvalidType("Object ne peut pas etre pere"))
*)
>>>>>>> .r54
...
=======
	 if Some(t) <> e.etype then raise(Typemismatch("Types non compatibles"))
>>>>>>> .r54
	|(None , name , None) -> ()
	|(None , name , Some e) -> ();
...
let type_attribut gen_env att = match att.adefault with
	|None -> ()
	|Some(e) -> checkTypeExpression gen_env { returntype = Type.Void; vars =Hashtl.create 1} e; let type_attribut = Some(att.atype) in verif_ref_type gen_env type_attribut e.etype;
...
=======
        | Some(Type.Ref(type_reel)) -> verif_ref_type gen_env type_apparent type_reel );
        | _ -> raise(Typemismatch("Utilisation non autorisée de l'opérateur "))
		
 | _ -> raise(UndefinedType("Type null ou non défini"));
>>>>>>> .r54
...
>>>>>>> .r54
 ...
 let arg_compare x y = 
 if  x.ptype <> y.ptype then raise(Typemismatch("Types non compatibles"));
...
||||||| .r48
	(* else raise *)
		ajouter_classe gen_env c
=======
	 else raise(UndefinedType("Type non défini"))
		ajouter_classe gen_env c;
>>>>>>> .r54
